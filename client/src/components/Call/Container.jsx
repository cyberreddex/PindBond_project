...
function Container({ data }) {
      ...
      import("zego-express-engine-webrtc").then(
        async ({ ZegoExpressEngine }) => {
          ...
          zg.on(
            ...
            async (roomID, updateType, streamList, extendedData) => {
              if (updateType === "ADD") {
                ...
                zg &&
                localStream &&
                streamList && // Check if streamList is defined
                streamList.length > 0 && // Ensure streamList is not empty
                streamList[0].streamID // Check if streamID exists
              ) {
          ...
          if (localVideo) {
            ...
            localVideo.appendChild(videoElement); // Safe append if localVideo exists

            const td = document.getElementById("video-local-zego");
            td.srcObject = localStream; // Set the local stream to the video element
          } else {
  ...
  return (
    <div className="border-conversation-border border-l w-full bg-conversation-panel-background flex flex-col h-[100vh] items-center justify-center text-white">
